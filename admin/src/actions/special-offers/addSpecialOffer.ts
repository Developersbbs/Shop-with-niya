...
export async function addSpecialOffer(
  ...
  try {
    ...
    const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/special-offers`, {
      method: "POST",
      headers: {
    ...
    if (!response.ok) {
      ...
      return {
        success: false,
        message: errorData.message || "Failed to create special offer",
    ...
    console.error("Error in addSpecialOffer:", error);
    return {
      success: false,
